...
describe('calculate', () => {
  ...
  it('omits b for unary requests', async () => {
    const fetchSpy = vi.fn().mockResolvedValue({
      ok: true,
      headers: { get: () => 'application/json' },
    ...
    await calculate({ op: 'sqrt', a: 9 });

    expect(fetchSpy).toHaveBeenCalledWith('/api/v1/calculations', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ op: 'sqrt', a: 9 }),
    });
  });
  ...
  it('throws with backend error message on non-200 response', async () => {
    ...
    await expect(calculate({ op: 'divide', a: 10, b: 0 })).rejects.toThrow(
  ...
  it('throws with backend message on 400 response', async () => {
    ...
    await expect(calculate({ op: 'add', a: 1, b: 2 })).rejects.toThrow(
      'operation must be one of add, subtract, multiply, divide',
    );
  });

  it('throws with backend message for advanced operation errors', async () => {
    mockFetch(422, {
      error: {
    ...
    await expect(calculate({ op: 'sqrt', a: -1 })).rejects.toThrow(
