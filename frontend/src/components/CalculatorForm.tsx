...
type UnaryOperationConfig = BaseOperationConfig & {
  value: UnaryOperation;
  arity: 'unary';
};

type BinaryOperationConfig = BaseOperationConfig & {
...
type OperationConfig = UnaryOperationConfig | BinaryOperationConfig;
...
export function CalculatorForm({ onSubmit, loading }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const isUnaryOperation = selectedOperation.arity === 'unary';

    if (a.trim() === '') {
      setValidationError(
        isUnaryOperation ? 'Operand is required.' : 'Both operands are required.',
      );
      return;
    }
    ...
    if (calcMin !== null && numB < calcMin) {
      setValidationError(`Operands must be at least ${calcMin}.`);
      return;
    }

    if (calcMax !== null && numB > calcMax) {
